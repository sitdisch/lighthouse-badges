...
import { getAverageScore, getSquashedScore, percentageToColor, proceed } from './calculations.js';

// Buffer size for stdout, must be big enough to handle lighthouse CLI output
const maxBuffer = 1024 * 50000;

export const metricsToSvg = async (lighthouseMetrics, badgeStyle, precedingLabel, outputPath) => {
...
export const calculateLighthouseMetrics = async (url, shouldSaveReport, additionalParams = '') => {
  const lighthouseBinary = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'node_modules', '.bin', 'lighthouse');
